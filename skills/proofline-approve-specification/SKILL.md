...
사용자가 검토한 REQ, 대상 AC와 REQ가 참조하는 Normative Design Documents를 승인하고 current canonical specification baseline을 형성한다. **사용자만** approval authority를 가지며 agent는 exact 내용을 제시하고 명시적인 결정을 기다린다.

## When to Use

- Confirmed Discovery에 연결된 REQ와 `criteria.create` AC를 승인할 때
- `criteria.update`, `criteria.retire` 또는 `criteria.satisfy` 집합을 승인할 때
- REQ의 `Normative Design Documents` exact contents를 같은 Specification baseline으로 검토할 때
...
REQ의 Objective·Scope·Non-Goals, 대상 AC의 Criterion·Verification과 REQ가 열거한 exact project-relative path의 exact document contents를 사용자에게 보여주고 명시적인 approval을 기다린다. 승인 없이 REQ를 `approved`, AC를 `active` 또는 `retired`로 전환하지 않는다.

Design documents는 별도 lifecycle status 또는 approval metadata를 갖지 않는다. `Normative Design Documents` 목록이 없는 REQ도 유효하며 design-document 부재만으로 approval을 거부하지 않는다. 목록이 있으면 모든 path가 repository 안의 existing regular Markdown file인지 확인하고 exact contents를 누락 없이 제시한다. Symlink, repository escape, missing path 또는 unexpected file type은 approval blocker다.
...
2. REQ `Constraints`의 `Normative Design Documents` 목록을 확인한다. 목록이 있으면 각 exact project-relative path의 type, repository boundary와 exact document contents를 읽는다.
3. REQ의 전체 `create`·`update`·`retire`·`satisfy` AC 변경 집합과 각 대상 AC의 exact 내용, 그리고 모든 referenced design-document exact 내용을 사용자에게 함께 제시한다.
4. 사용자의 명시적인 approval 또는 correction을 기다린다.
5. Approval 뒤 REQ를 `approved`, `criteria.create`·`criteria.update` AC를 `active`, `criteria.retire` AC를 `retired`로 전환한다. `criteria.satisfy` 대상의 본문과 status는 변경하지 않는다. Design documents에는 status를 기록하지 않는다.
...
5. **Referenced design document를 요약만 제시함.** Exact path와 exact document contents 전체가 approval 대상이다.
6. **Design document에 status를 추가함.** Lifecycle status는 REQ/AC만 소유한다.

## Verification Checklist

- [ ] Discovery가 confirmed이다.
- [ ] REQ와 대상 AC exact 내용을 사용자에게 제시했다.
- [ ] 모든 Normative Design Documents exact path와 exact document contents를 함께 제시했다.
