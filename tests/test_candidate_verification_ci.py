...
AGENT_SKILL_TESTS = ROOT / "tests/test_agent_skills.py"
...
def test_candidate_workflow_has_exact_required_jobs_and_hardened_checkout_first() -> (
    None
):
    workflow = _workflow()
    jobs = workflow["jobs"]
...
def test_ubuntu_and_windows_independently_verify_same_wheel_and_required_regressions() -> (
    None
):
    text = WORKFLOW.read_text(encoding="utf-8")
    workflow = _workflow()
    ...
    assert (
        'uv pip install --python "$env:RUNNER_TEMP\\proofline-wheel-env\\Scripts\\python.exe"'
        in text
    )
    assert (
...
def test_package_resources_and_retired_home_boundary_are_documented() -> None:
    assert "package" in README.read_text().lower()
    assert "package" in STORAGE_CONTRACT.read_text().lower()
    assert "~/.proofline/" in README.read_text()
    assert "생성·읽기·갱신·삭제하지 않는다" in STORAGE_CONTRACT.read_text()
    assert "proofline_resources" in ARTIFACT_LAYOUT.read_text()


def test_each_hosted_job_contains_repository_external_command_state_and_no_mutation_checks() -> (
    None
):
    workflow = _workflow()
    required = (
    ...
    for job_name, job in workflow["jobs"].items():
        ...
        tracked = [
            match.start()
            for match in re.finditer("status --short --untracked-files=all", runs)
        ]
        ignored = [
            match.start()
            for match in re.finditer(
                "status --short --ignored --untracked-files=all", runs
            )
        ]
        assert len(tracked) >= 2 and len(ignored) >= 2, job_name
        command = (
            runs.index("uv build --wheel")
            if job_name == "build-candidate"
            else runs.index("pytest")
        )
    ...
    for job in workflow["jobs"].values():
        for step in job["steps"]:
            ...
            if "python -m compileall" in run:
                assert run.index("PYTHONPYCACHEPREFIX") < run.index(
                    "python -m compileall"
                )
...
def test_verified_absolute_wheel_is_exported_to_each_hosted_consumer_suite() -> None:
    ...
    assert (
        ubuntu.index("sha256sum --check --strict SHA256SUMS")
        < ubuntu.index("WHEEL=$(realpath proofline-*.whl)")
        < ubuntu.index('export PROOFLINE_HOSTED_CANDIDATE_WHEEL="$WHEEL"')
    )
    assert ubuntu.index(
        'export PROOFLINE_HOSTED_CANDIDATE_WHEEL="$WHEEL"'
    ) < ubuntu.index('uv run pytest -q -m "not candidate_build_only"')
    assert (
        windows.index("candidate provenance mismatch")
        < windows.index("$env:PROOFLINE_HOSTED_CANDIDATE_WHEEL = $wheel[0].FullName")
...
def test_hosted_wheel_consumers_validate_exact_file_and_bypass_local_build(
    ...
    assert provenance_source.index("returncode == 0") < provenance_source.index(
        return_statement
    )
...
def test_windows_gate_exercises_exact_wheel_and_full_fresh_install_sequence() -> None:
    ...
    checksum_block = text[
        text.index("$ChecksumLines") : text.index('Write-Output "candidate-wheel')
    ]
...
def test_windows_gate_pins_skill_resources_and_avoids_retired_home() -> None:
    text = WINDOWS_GATE.read_text(encoding="utf-8")
    for marker in (
        "files('skills')",
        "status --json",
    ...
    assert "package skill inventory" in workflow


def test_windows_gate_does_not_read_or_mutate_line_execution_status(
    tmp_path: Path,
) -> None:
    ...
    subprocess.run(
        ("git", "config", "user.name", "ProofLine Gate"), cwd=project, check=True
    )
    subprocess.run(
    ...
    subprocess.run(
        ("git", "commit", "-qm", "fixture baseline"), cwd=project, check=True
    )
...
def test_workflow_and_gate_preserve_governance_and_agent_state_boundaries() -> None:
    workflow = WORKFLOW.read_text(encoding="utf-8")
    gate = WINDOWS_GATE.read_text(encoding="utf-8")
    assert ".proofline/lines" not in workflow
    assert "git tag" not in gate.lower()
    tests = AGENT_SKILL_TESTS.read_text(encoding="utf-8")
    assert "state_root" in tests
    assert "preserve_unrelated_skill" in tests
...
def test_hosted_workflow_declares_shared_clean_runner_plan_contract() -> None:
    ...
    assert contract == {
        ...
        "PROOFLINE_CLEAN_RUNNER_STEP_ORDER": (
            ...
            "install-candidate,provision-test-dependencies,contract-probe"
        ),
    ...
    for platform in ("ubuntu-python311", "windows-python311"):
        ...
        provision = next(
            step for step in steps if step["step_id"] == "provision-test-dependencies"
        )
        assert provision["endpoint"] == contract["PROOFLINE_CLEAN_RUNNER_ENDPOINT"]
        assert (
            provision["version_source"]
            == contract["PROOFLINE_CLEAN_RUNNER_VERSION_SOURCE"]
        )
        assert (
            provision["network_mode"] == contract["PROOFLINE_CLEAN_RUNNER_NETWORK_MODE"]
        )
